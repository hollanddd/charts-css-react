 ...
 */
export type ChartTypes = 'bar' | 'column' | 'area' | 'line' | 'pie' | 'donut' | 'radar' | 'polar';
/**
 * Represents properties for creating a new chart.
 */
export interface IChartProps extends HTMLAttributes<HTMLTableElement> {
  /**
   * The type of chart.
   *
   * @default bar
...
export const Chart: FC<ChartProps> = ({
  ...
  if (showSecondaryAxes) variations.push(`show-${showSecondaryAxes}-secondary-axes`);
 ...
 */
export interface DataProps extends HTMLAttributes<HTMLTableDataCellElement> {
  /**
 ...
 * element.
 */
export const Data: FC<DataProps> = ({ size, start, children, ...rest }) => {
  let style = { '--size': size, '--start': start }
  return <td style={style as CSSProperties} {...rest}>{children}</td>
};
/**
 * Tooltip
 */
export const Tip: FC<{}> = ({ children }) => <span className="tooltip">{children}</span>;
/**
 ...
 *
 */
export const Legend: FC<LegendProps> = ({ labels, shape, orientation, ordered=false }) => {
  ...
  return ordered
    ? <ol className={variant}>{labels.map(label => <li>{label}</li>)}</ol>
    : <ul className={variant}>{labels.map(label => <li>{label}</li>)}</ul>
};
/**
 * Adds table head
 */
export const Header: FC<{labels: string[]}> = ({labels}) => (
  <thead><tr>{labels.map((label: string) => <th scope="col">{label}</th>)}</tr></thead>
);
